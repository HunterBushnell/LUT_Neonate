...
class FeedbackLoop(SimulatorMod):
    ...
    # Grill, et al. 2016
        def blad_vol(vol):
            f = 1.5*7*0.5*(1400/max_v)*vol - 10   #math.exp(48*vol-64.9) + 8
            f = max(f,0.0)
            return f
    ...
    ### STEP 3: Volume Calculations ###
        ...
        void = 1000 		# 10x for sim, 100uL/s no value, just using some logic
        void /= (1000) # Scale from ul/s to ul/ms
        max_v = 250 		# For neonatal, it depends. P1-P7 = 82.5uL (Z and Z 2012)
        vol = v_init
    ...
    ### STEP 4: Pressure and Bladder Afferent FR Calculations ###
        ...
        if self.fill and (block_interval[0]*sim.dt)%1000 == 0:
          # randnum = np.random.uniform(0,(vol/max_v))
          p = p + 3*(vol/max_v)#(np.random.uniform(randnum*(vol/max_v),(vol/max_v)))
        else:
          p=p
